...
public class RollingChecksum {
	...
	public RollingChecksum(File f, int blockSize) {
		...
		try {
			...
			throw new RuntimeException(String.format("Failed to access file [%s]", f.getAbsolutePath()), e);
		}
	}
		...
		if (index < data.length - blockSize) {
			...
			if (stream != null && !eofReached) {
				...
				try {
					...
					throw new RuntimeException(String.format("Failed to read or close file [%s]", f.getAbsolutePath()), e);
				}
			}
